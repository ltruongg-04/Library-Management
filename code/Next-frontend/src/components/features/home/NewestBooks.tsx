    ...
    return (
        ...
        >
            <div className="relative aspect-[3/4] w-full overflow-hidden rounded-xl bg-surface-container-low dark:bg-slate-800">
                ...
                {/* NEW tag */}
                <div className="absolute left-2 top-2 flex items-center justify-center rounded bg-primary-500 px-2 py-0.5 text-[10px] font-bold text-white shadow-sm">
                    {UI_TEXT.BOOK_DETAIL.NEW_BADGE}
                </div>
            </div>
...
export default function NewestBooks() {
    ...
    useEffect(() => {
        ...
        const fetchBooks = async () => {
            try {
                const response = await bookService.getBooks({ sortBy: "newest", size: 6 });
                if (isMounted) {
