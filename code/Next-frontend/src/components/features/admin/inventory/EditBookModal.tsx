...
export default function EditBookModal({ bookId, isOpen, onClose, onSuccess }: EditBookModalProps) {
    ...
    const fetchBookDetails = async () => {
        try {
            ...
            setSelectedTags(data.tags?.map((t) => ({ value: t.id.toString(), label: t.name })) || []);
            ...
            setError(err.message || API_ERRORS.FETCH_BOOK_ERROR);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.message || API_ERRORS.SAVE_BOOK_ERROR);
        } finally {
            setSaving(false);
        }
    };
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <CreatableSelect
                                        isMulti
                                        options={availableTags.map((t) => ({ value: t.id.toString(), label: t.name }))}
                                        value={selectedTags}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        value={quantity}
                                        disabled
                                        className="w-full cursor-not-allowed rounded-lg border border-surface-container-high bg-surface-container-low px-3 py-2 text-[14px] text-on-surface-variant"
                                    />
